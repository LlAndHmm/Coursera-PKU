...
Headquater::Headquater(string color_, int M) {
		elements = M;
		color = color_;
...
//判断主武器类型 
char* Headquater::PrimaryWeapon() {
	if (nWarrior % 3 == 0) {
		return "sword";
	}
	else if (nWarrior % 3 == 1) {
		return "bomb";
	}
	else {
		return "arrow";
...
char* Headquater::SecondaryWeapon() {
	if ((nWarrior + 1) % 3 == 0) {
		return "sword";
	}
	else if ((nWarrior + 1) % 3 == 1) {
		return "bomb";
	}
	else {
		return "arrow";
...
//计算士气
float Headquater::CalculateMorale()  {
	float morale = (float)elements / (float)dHP; 
	return morale;
}
bool Headquater::produceIceman() {
	if (iHP <= elements) {
		...
		cout << "It has a " << PrimaryWeapon() << endl;	
		return true;
	}
	else return false;
}
bool Headquater::produceNinja() {
	if (nHP <= elements) {
		...
		<< nHP << "," << nNinja << " ninja in "
		<< color << " headquarter" << endl; 
		cout << "It has a " << PrimaryWeapon() <<
		 " and a " << SecondaryWeapon() << endl;	
		return true;
	}
	else return false;
}
...
bool Headquater::produceDragon() {
	if (dHP <= elements) {
		...
		setprecision(2) << fixed << CalculateMorale() << endl;
		return true;	
	}
	else return false;
}
